...
layui.define(['table', 'admin', 'view', 'table', 'form', 'jquery'], function (exports) {
    ...
    $(function() {
        ...
        if(type == 'teacher') {
            cols = [[
                ...
                , {field: 'score', align: 'center', title: '评分', templet: "#scoreTpl"}
                , {field: 'score', align: 'center', title: '创建时间', templet: function (d) {
        ...
        table.render({
            ...
            ,url: type == 'teacher' ? '/api/factory/getTeacherList' : '/api/factory/getFactoryHRList'
            ,method: "post"
            ,cellMinWidth: 80
            ,where: {"id":data.fid}
            ,cols: cols
        ...
        form.on('submit(allotTeacherAdd)', function (obj) {
            ...
            admin.req({
                url: type == 'teacher' ? "/api/factory/allotTeacher" : '/api/factory/allotFactoryHR'
                ,type: 'post'
                ,data: valud
        ...
        table.on('tool(LAY-factory-allot-list)', function(obj) {
            ...
            if (obj.event == 'factory-allot-del') {
                ...
                admin.req({
                    url: type == 'teacher' ? "/api/factory/delTacher" : '/api/factory/delFactoryHR'
                    ,type: 'post'
                    ,data: {
                        fid: obj.data.fid,
                        uid: obj.data.uid
    ...
    function getTeacher(fid) {
        admin.req({
            url: type == 'teacher' ? "/api/factory/getAvailableTeacher" : "/api/factory/getAvailableFactoryHR"
            ,type: 'post'
            ,data: {"fid":fid}
            ,success: function(res) {
                var users = res.data;
