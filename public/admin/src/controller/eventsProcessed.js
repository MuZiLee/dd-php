...
layui.define(['jquery', 'element', 'layer', 'laydate', 'layedit', 'view', 'table', 'form', 'admin', 'upload'], function(exports) {
    ...
    table.render({
        elem: "#LAY-eventsProcessed-list"
        , url: '/api/events/getEventRolesProcessed'
        , method: "post"
        , cols: [[
            ...
            // , {title: "操作", width: 100, align: "center", fixed: "right", toolbar: "#LAY-eventsProcessed-list-tool-tages"}
        ]]
        , cellMinWidth: 100
        // ,height: 'full-200'
        , page: true
    ...
    table.on("tool(LAY-eventsProcessed-list)", function(obj) {
        if(obj.event === "rocessed-edit") {
            ...
            admin.popup({
                title: '备注'
                , area: ['30%', '40%']
                , id: 'LAY-eventsProcessed-list-remarkTpl'
    ...
    form.on('submit(form-roles)', function(data){
        ...
        //执行重载
        table.reload('LAY-eventsProcessed-list', {
            url: '/api/events/getEventRolesProcessed'
            ,type: "post"
        });
    });
    ...
    form.on('submit(form-signUp)', function(data){
        ...
        //执行重载
        table.reload('LAY-eventsProcessed-list', {
            url: '/api/events/getSignUpProcessed'
            ,type: "post"
        });
    });
    //打卡
    form.on("submit(form-punchCard)", function(data) {
        ...
        //执行重载
        table.reload('LAY-eventsProcessed-list', {
            url: '/api/events/punchCardProcessed'
            ,type: "post"
        });
    });
    //报销
    form.on("submit(form-writeOff)", function(data) {
        ...
        //执行重载
        table.reload('LAY-eventsProcessed-list', {
            url: '/api/events/writeOffProcessed'
            ,type: "post"
        });
    });
    //请假
    form.on("submit(form-askForLeave)", function(data) {
        ...
        //执行重载
        table.reload('LAY-eventsProcessed-list', {
            url: '/api/events/askForLeaveProcessed'
            ,type: "post"
        });
    });
    //离职
    form.on("submit(form-resign)", function(data) {
        ...
        //执行重载
        table.reload('LAY-eventsProcessed-list', {
            url: '/api/events/resignProcessed'
            ,type: "post"
        });
    });
    //提现申请
    form.on("submit(form-cashIn)", function(data) {
        ...
        //执行重载
        table.reload('LAY-eventsProcessed-list', {
            url: '/api/events/cashInProcessed'
            ,type: "post"
        });
    });
    //预支申请
    form.on("submit(form-advance)", function(data) {
        ...
        //执行重载
        table.reload('LAY-eventsProcessed-list', {
            url: '/api/events/advanceProcessed'
            ,type: "post"
        });
    });

    form.render(null, 'LAY-eventsProcessed-form-list');
    ...
    // function eventsProcessedPOP({success: success}) {
    //     layer.prompt({
    //         formType: 1
    //         , title: "敏感操作，请验证口令"
    ...
    // var eventsProcessedList = function() {
    //
    // };
    //
    //
    // var objs = {
    //     eventsProcessedList: eventsProcessedList
    // };
    exports("eventsProcessed", {});
});